...
//! single location and `tomte why --all` lists everything. `--json` emits the
//! file's decisions as a JSON array for structured piping.

use anyhow::Result;
use tomte_core::decisions;

pub async fn run(file: String, json: bool, cwd: Option<std::path::PathBuf>) -> Result<()> {
    if let Some(dir) = &cwd {
