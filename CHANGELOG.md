...
- On Windows, `run_shell` commands now run inside a Job Object with `KILL_ON_JOB_CLOSE`, so a timed-out or killed command's whole process tree is torn down (Windows has no Unix process groups to kill). Best-effort cleanup only: filesystem and network stay unconfined there, and `doctor` still reports the platform as unsandboxed.
...
- Fixed the `memory` tool's `view` panicking the turn when given a reversed `view_range` such as `[2, 0]`: the slice now requires `start <= end` and returns an error, so a model-supplied (or prompt-injected) range can't wedge the session.
- Fixed `grep`/`glob` panicking on a `head_limit` near `usize::MAX`: the line-slice upper bound is now a saturating add, so an extreme paging value returns the clamped output instead of crashing the turn.
- Hardened `web_search` against SSRF: redirects are now disabled (the old guard only rejected redirect targets that were literal IPs, not hostnames resolving to an internal address), and any result URL whose host is a literal internal/metadata IP is dropped before reaching the model or a later `web_fetch`.
- Redacted bare-JWT OAuth access tokens from provider error bodies: ChatGPT/Codex tokens carry no `sk-`/`Bearer ` prefix for the heuristic to catch, so the OpenAI/Anthropic OAuth error paths now also redact the credential by exact value (the custom-provider API-key path already did).
- Fixed auto-compaction disarming itself for the rest of a session when the summary request failed: a failed or no-op `/compact` now re-arms the once-per-window guard, so a later 85% tick can retry instead of letting the session drift into a hard context overflow.
- Fixed Anthropic streaming usage accounting dropping a corrected cache or input-token count from `message_delta`: the final usage now folds every field the delta reports over the `message_start` snapshot, not just `output_tokens`, so `/cost` and the context-occupancy estimate stay accurate.
- Fixed a mid-stream context overflow that arrived *after* answer text had streamed re-running the turn and showing the answer twice: the shed-and-retry recovery is now gated on no committed output, matching the fail-over path's existing "never duplicate a partial answer" invariant.
- Stopped auth redaction from mangling benign words in error output: `sk-`/`Bearer ` are now matched only at a word boundary, so `disk-usage`, `risk-free`, or `ask-them` survive intact while a real token (after a quote, `=`, `:`, or space) is still redacted.
- Fixed the `run_shell` danger classifier missing `dd of=/dev/vd*` (virtio — the default disk on KVM/QEMU/cloud VMs) and numbered `/dev/disk[0-9]` (macOS): the `dd` check now shares the redirect guard's device-family list instead of a bespoke one that had drifted out of sync.
- `auth.json` permissions now self-heal on load: if the credential file is group/world-readable (restored from a backup, copied with `cp -p`, or placed via `XDG_CONFIG_HOME`), `load_auth` tightens it back to owner-only `0o600` instead of reading tokens other local users could see.
- Fixed a stray request to the OAuth loopback callback (`127.0.0.1:1455`) aborting an in-progress sign-in: a callback whose `state` doesn't match the login nonce — a browser prefetch, a probe, or a forged request — is now ignored instead of consuming the result channel and shutting the local server down.
- Fixed ChatGPT/Codex OAuth sign-in failing with `authorize_hydra_invalid_request`: the loopback `redirect_uri` was sent as `127.0.0.1`, but the OAuth client whitelists `localhost` by exact match. It now sends `localhost` while the listener still binds `127.0.0.1`.
- Fixed bracketed paste being dropped on the login screen, so pasting the Claude OAuth code (or an API key) did nothing: the paste event was routed only to the chat composer. It now lands in the active login field, with CR/LF stripped.
...
- Unified the TUI's ~70 scattered color literals into one calm palette module: an achromatic base, a single muted sage-teal accent, and muted semantic colors for diff/status. Per-provider auth dots, context-usage swatches, and the `/buddy` pet stay deliberately distinct.
...
- Added automatic, provider-agnostic model failover: when the active model is rate-limited or overloaded — before the stream opens or via a mid-stream overload event — the turn switches to the next model in `fallback_models` and continues. Off by default; only fails over before any answer text has streamed (so output is never replayed) and never to a provider you aren't signed in to.
