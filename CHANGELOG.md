...
- Added automatic, provider-agnostic model failover: list models in `fallback_models` in `config.json` (each a spec in the same form as `model` — a different provider or a local endpoint is fine) and when the active model is rate-limited or its provider is overloaded, the turn transparently switches to the next configured fallback and continues instead of failing. Off by default (empty list, so existing setups are unchanged). It's bounded, only triggers on a genuine overload/rate-limit (never a fatal/auth/refusal or context-overflow error), skips a fallback whose provider has no stored credential (so a rate-limit can't degrade into a confusing auth error), and never fails over after output has already started streaming. The TUI and headless modes surface a notice and adopt the fallback as the session model.
- The `/cost` report is now accurate and per-model. Token spend is tallied separately for each model used in the session — a mid-session `/model` switch now bills each model at its own rate instead of pricing the whole session at the active model — and split by billing class (fresh input, output, cache read, cache write) so a cache-heavy Anthropic session is no longer billed as if every cached-read token were full-price input. Anthropic models (Opus/Sonnet/Haiku, including dated snapshots) are now priced from their own published rates instead of falling through to a GPT-5 default, and the running tally is persisted in the session record so `/cost` survives a `/resume`.
- Added project-scoped configuration: a `.opencli/config.json` in the working directory now overrides the global `config.json` for that project. Because the file ships in cloned repos, only safe behavioral fields are honored — `model`, `reasoning_effort`, `verbosity`, `auto_compact`, and `fallback_models`. Security-sensitive keys (`default_permission_mode`, `auto_approve_read`/`auto_approve_write`, `providers`) are ignored in a project file (with a warning) and remain global-only, so a cloned repo cannot silently disable approval prompts, auto-approve writes, or redirect the model to another endpoint. The project file is size-bounded and a malformed one is ignored rather than resetting settings.
- Added `opencli-website/` — the project's marketing & documentation site (Next.js App Router, fully static), deployed to Vercel with automatic deploys on every push to `main`. Because the site lives in a subdirectory of this monorepo, the Vercel project's Root Directory is set to `opencli-website` (otherwise the build would run against the Rust crate at the repo root). Live at https://opencli-website.vercel.app.
- Rebuilt `/context` as a visual context-window breakdown in the style of Claude Code's `/context`: a proportional grid colored by category, a per-category legend (base system prompt, built-in tool schemas, custom agents, inherited memory files, skills, and the visible conversation — each with estimated tokens and share of the window), free space, and detail sections for MCP servers, custom agents, memory files, and skills with their slash-command shortcuts. Counts (agents, skills, memory files, MCP servers) are exact; the per-category token figures are chars/4 estimates of the real source text, and the provider-reported occupancy from the last turn is shown as the authoritative number. `/context all` expands the detail sections to list the actual agents, memory files, and skills. Backed by a new `opencli_core::context_report` estimation module (unit-tested) and a `Block::Rich` scrollback variant that renders pre-styled colored lines verbatim.
- Added `opencli doctor` and the `/doctor` slash command — a read-only setup health check. It verifies authentication (signed-in mode, `auth.json` presence and owner-only `0600` permissions, and a credential-coverage matrix per provider), configuration (config file, project overlay, valid `reasoning_effort`/`verbosity`, and that each configured custom provider has a resolvable API key), and model routing — cross-checking the configured model against available credentials, so the common "model points at a provider you aren't signed in to" mistake now surfaces as a clear error instead of an opaque 401 mid-turn. It also reports configured MCP servers (flagging any whose launch command isn't on `PATH`), discovered skill/subagent/hook counts, and the external tools opencli shells out to (`git`, `ripgrep`, `grep`). The check is side-effect-free — it never writes anything or spawns an MCP server — and runs headless (no TUI, no model call), so it works even when startup is broken; `opencli doctor` exits non-zero if any check fails, so it can gate a setup script or CI step.
...
- `run_shell` deny rules are harder to bypass: quoted command names (`"rm"`, `r''m`), wrapped commands hidden behind flags or arguments (`sudo -u root rm`, `timeout 5 rm`), command substitution, backticks, and subshells now still hit `deny(rm:*)`.
...
- A malicious or mistyped path-glob permission rule can no longer hang the agent: runs of `*` in a rule (`***`, `**********`) are collapsed to a single `**` before matching, eliminating the `O(n^k)` backtracking that a crafted `.opencli/permissions.json` (which ships in cloned repos) could otherwise trigger on every tool call.
- Project `.opencli/permissions.json` is no longer trusted to *grant* permissions: its `allow` list is ignored (only `deny` is honored), so a cloned repo can tighten what the agent may do but can never pre-approve silent tool execution — closing the same untrusted-repo hole that `.opencli/config.json` already guards against. A user's own "allow in this project" choices now persist in an owner-only, per-project store under the config directory (outside the repo) instead of the in-repo file, so they still survive across sessions but cannot be seeded by a checked-in file. (Any pre-existing in-repo `allow` rules simply stop taking effect and are re-granted on the next prompt.)
- `run_shell` deny rules now also catch brace groups (`{ rm -rf /; }`) and glued redirections (`curl>out`, `rm<x`), which previously slipped past `deny(rm:*)`/`deny(curl:*)` because the program name was hidden behind `{` or fused with a `>`/`<` character.
- The secret-environment scrub applied before every `run_shell` now also strips `DATABASE_URL` (which routinely embeds inline credentials), bare `*_KEY` (e.g. `STRIPE_KEY`), `*_PWD` (e.g. `MYSQL_PWD`), `*_DSN`, and `*WEBHOOK*` variables — common credential names the previous `*_TOKEN`/`API_KEY`-style list missed.
- `/cost` no longer overstates OpenAI spend: cached input tokens reported by the OpenAI Responses API (`input_tokens_details.cached_tokens`, which OpenAI folds into `input_tokens`) are now split out and billed at the cache-read rate instead of full input rate, matching how Anthropic cache reads are already handled. Context-occupancy math is unchanged.
...
- Context-warning and auto-compact thresholds now handle extreme provider token counts safely, so a malformed `usage` payload can't crash debug builds or make release builds miss the warning.
...
- `web_fetch` now pins the exact DNS addresses that passed the SSRF check into the HTTP client, so a hostile DNS server can't pass validation with a public address and then connect to a private one.
- Web search response bodies are now capped at 8 MiB while streaming, so a bad or compromised search backend can't grow memory without bound before parsing results.
- MCP server messages are now capped at 16 MiB per JSON-RPC line before parsing, so a buggy or malicious MCP subprocess can't exhaust memory with one newline-less response.
