...
- Fixed pasting an **image** on Windows — a `Win+Shift+S` screenshot (or any clipboard bitmap) now attaches via Alt+V. arboard's CF_DIB reader misses Snip & Sketch / screenshot bitmaps, so on Windows tomte now reads the clipboard image through PowerShell's `System.Windows.Forms.Clipboard::GetImage()` → `Save(.., Png)` (the same mechanism Claude Code uses); macOS and Linux keep arboard's native reader, which needs no external binary. Alt+V also says when the clipboard has nothing to paste instead of silently doing nothing.
...
- Fixed in-app text selection drifting onto the wrong text when you scroll — the highlight is no longer re-tracked by screen row (which mis-aligned as the content moved); it now clears on scroll, so a copy always matches what was highlighted. The full-screen alternate screen stays the default (input pinned to the bottom); the inline native-scrollback renderer remains opt-in via `TOMTE_INLINE=1`.
- Fixed the TUI getting laggier the longer a session ran — the alt-screen renderer re-wrapped and cloned the *entire* transcript every frame, so per-frame cost grew with history and showed up as lag while streaming (a full-transcript clone at ~60fps), on idle redraws, and on every mouse-move event. It now materializes only the visible window — O(viewport) per frame instead of O(transcript) — reuses the cached prefix while streaming instead of cloning it, and stores the prefix split as an index so the transcript's wrapped lines live in memory once instead of twice (which also cuts the per-frame allocation churn). Mirrors how Claude Code's Ink `<Static>` renders history once and only redraws the live region.
- Fixed `glob`/`grep` hanging for minutes on a machine without ripgrep (the common case on Windows) — the no-ripgrep fallback walked the whole tree skipping only `.git`, so every pattern crawled `node_modules`, `target`, `.next`, `build` and every other heavy directory (tens of thousands of dependency files). It now honors `.gitignore`/`.ignore` (via the `ignore` crate, the same engine ripgrep uses), so those directories are skipped and the fallback is fast and correct whether or not `rg` is installed — `glob`/`grep` no longer depend on ripgrep for usable performance.
- Added a first-run setup card — on startup tomte auto-detects your OS and package manager and, if an important external tool is missing (currently `git`, which worktree isolation and repo-root memory shell out to), shows the exact install command for your platform (`winget`/`scoop`/`choco` on Windows, `brew` on macOS, `apt`/`dnf`/`pacman`/… on Linux, or a download link when no manager is found). It only shows the command — it never runs an installer — and no card appears when the environment is ready. `tomte doctor`'s now-stale note was corrected too: the no-ripgrep fallback is gitignore-aware (skips `node_modules`/`target`), not a dumb full-tree walk.
