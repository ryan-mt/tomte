...
- Gave a finished sub-agent in the fleet view a settled past-tense verb — a done teammate now reads e.g. `Forged · 4 steps · 1m 12s` (a stable per-agent pick from a tomte-voiced hearth-and-workshop pool) instead of a stale in-flight phrase, mirroring Claude Code's idle-teammate convention, while a still-running one keeps showing its live activity
- Added a live context gauge to the status line — once a turn reports usage, the footer shows `N% ctx` next to the model, colored calm → warning → danger as the window fills toward the ~85% auto-compact threshold (the occupancy indicator Claude Code keeps in its status line, reimplemented in tomte's calm palette); hidden on the first screen, before any usage
- Rebuilt the welcome card into a complete first-screen panel — the account's pixel-pet sits beside a banner carrying the brand and version, the live setup (`model · effort · account`), the workspace path, a getting-started line that flips to a ✓ once the project has house rules (`AGENTS.override.md`/`AGENTS.md`/`CLAUDE.md`, mirroring Claude Code's `/init` check), and a quiet shortcuts footer; the panel is sized to the pet's height and to its own content so the right edge stays populated (the version is pinned to the border) instead of trailing off into empty space, and over-long values are trimmed to fit a narrow terminal — the pet is the same rarity-rolled identity as `/buddy`, chosen once at startup
- Rewrote the `edit_file` / `multi_edit` diff into a real hunk (reverse-read from Claude Code's structured-patch renderer, reimplemented as tomte's own minimal prefix/suffix line-diff) — shared leading and trailing lines now collapse into uncolored context instead of being echoed as both removed and added, the `-`/`+` summary counts reflect only the lines that actually changed, and line numbers follow the unified-diff convention (context and added lines carry the new-file number, removed lines the old) so the gutter stays honest as the two sides diverge
- Rebuilt the turn spinner in a tomte voice — a pool of ~190 cozy hearth-and-workshop present-tense words (every entry unique) that holds one word for ~8s then drifts on a pure `seed × elapsed` schedule, so it never flickers and a short turn shows a single word; when a todo is in progress its `active_form` takes the line instead (matching Claude Code's `activeForm ?? verb`)
- Made the spinner words configurable on Claude Code's schema — `spinner_verbs { verbs, exclude_default }` in `config.json` appends to (or, with `exclude_default`, replaces) the built-in pool, resolved once at startup
- Gave the composer a cozy face — the prompt gutter is now `✿ ` and the empty-input placeholder reads `what shall we build today?`
- Fixed the drag text-selection drifting off its content on mouse-wheel scroll — the highlighted rows now shift with the scroll (`±3` per wheel step) instead of being cleared, so the selection stays anchored to the text it covers
- Tightened the opt-in inline viewport — a slimmer height (`clamp(rows / 3, 13, 16)`), a bottom-anchored live tail so the newest output hugs the input, and a lone welcome card that stays live in the viewport instead of being committed to scrollback
- Fixed sign-in on Windows — `save_auth` failed by design on every non-Unix platform ("secure auth storage requires owner-only file permissions"), so an OAuth login that succeeded in the browser could never persist and the TUI stayed stuck on the sign-in picker; Windows now writes `auth.json` under `%APPDATA%\tomte` and tightens it to the current user only (strips inherited ACEs and grants the owner via `icacls` — the analogue of the Unix `0o600`), while genuinely exotic non-Unix/non-Windows targets keep the explicit fail-closed error
- Surfaced a file's recorded decisions as "house rules" in the pre-flight (SOUL Pillar 5, A2 Tier 1) — when an `edit_file`/`write_file`/`multi_edit` targets a file that has recorded decisions, the glass-box card now lists them (up to three most-recent, with a `+k more · tomte why <file>` overflow), so the agent re-reads its own recorded constraints at the instant it could break one; pure surfacing — recall, not detection — so it can never be wrong and never gates
...
- Settled on the full-screen alternate-buffer renderer as the default — after trialing an inline default, the out-of-the-box layout is the conventional one (transcript above, input pinned to the bottom edge, in-app scroll and drag-selection); the inline viewport (SOUL Pillar 4 — finished turns flow into the terminal's own native scrollback so its scroll and copy keep working, only the live turn stays on screen) is now opt-in via `TOMTE_INLINE=1`
...
- Tightened Linux read-only sandbox writes — `/dev/shm` is no longer always writable, closing a tmpfs persistence escape while keeping ordinary device redirects such as `/dev/null` available
...
- Added runnable, std-only previews of all five SOUL pillars under `docs/previews/` (hand-compiled with `rustc`, invisible to cargo/CI, zero release risk) — the newest, `conscience.rs`, demos the proposed Pillar-5 active decision trail: it reconciles the trail to the code, confronts an edit that would reverse a past decision, and logs the human override
- Added a `cost_receipts.rs` preview of the cross-provider cost enabler (SOUL §5) — real per-billing-class token arithmetic across an OpenAI + Anthropic session reconciled into one normalized bill, the multi-model cost view a single-vendor tool can't show
...
- Extended secret-env scrubbing to read-only helper subprocesses — `grep`/`glob` search helpers, startup git-root discovery, worktree git helpers, the `@` file picker, and `/diff` now drop token/key/password/agent-socket environment variables before spawning
...
- Fixed the read-before-overwrite guard — failed `read_file` calls such as `limit: 0`, over-cap limits, or large-file-without-limit no longer mark the file as read, so `write_file` / `edit_file` cannot clobber content the model never actually saw
...
- Closed a ReDoS in the path-permission glob matcher — an untrusted `.tomte/permissions.json` deny rule with adjacent `**` groups (`**a**a…`) backtracked `O(text^k)` and could hang the agent on the first file-tool call; the matcher is now memoized to `O(pattern·text)`
- Bounded skill discovery against a symlink fan-out — a cloned repo's `.tomte/skills/` full of self-referential symlinks no longer explodes the startup walk to billions of directory reads (a visited-set caps it while still following legitimate symlinked skill dirs)
- Closed a sub-agent confinement bypass — a project-local agent matched only by its frontmatter `name` (a planted `innocuous.md` with `name: deploy`) is now confined to read-only tools, instead of running mutating tools under Auto / `--dangerously-skip-permissions`
- Made the Linux sandbox fail closed when Landlock is inactive — a kernel without active Landlock (compiled out, off the boot `lsm=` list, or many containers) silently enforced nothing, so commands ran with no filesystem confinement; they are now refused, and `tomte doctor` probes `/sys/kernel/security/lsm` and warns instead of claiming writes are confined
- Made the sandbox fail closed when its confinement helper can't be built — if it can't locate this executable to re-exec, a confining mode now refuses to run rather than silently falling back to an unconfined shell
- Hardened the `run_shell` destructive-command guard — an interpreter hidden behind shell grouping or `exec` (`curl … | { sh; }`, `| (sh)`, `| exec sh`) is now flagged, as are raw-block-device redirects using `>|`, bash `&>`, or a spaced/fd-prefixed operator (`echo x >| /dev/sda`, `… 2> /dev/sda`)
- Closed an SSRF gap — the whole `0.0.0.0/8` "this-network" range is blocked (not just `0.0.0.0`), since some stacks route the rest of it to the local host
- Tightened the read-before-overwrite guard — a partial (`offset`/`limit`) read no longer authorizes `write_file` to overwrite a file, so it can't discard lines the model never saw (a full read or a freshly written file still does; `edit_file` is unaffected)
- Fixed resume dropping Anthropic reasoning — signed / `redacted_thinking` blocks are now persisted in the session (they were `#[serde(skip)]`), so a resumed turn replaying a `tool_use` no longer loses its required thinking block and 400s; the OpenAI wire still omits them
- Fixed a token-refresh lockout — when persisting freshly refreshed OAuth tokens fails, the in-flight turn proceeds on the new access token (with a warning) instead of dying and leaving the next turn to hit the already-consumed single-use refresh token
- Defanged framework block markers in inherited docs — a planted `CLAUDE.md`/`AGENTS.md` carrying a literal tomte block marker can no longer truncate the prompt's inherited-memory block
- Fixed the Anthropic refusal error losing its explanation when a trailing usage-only `message_delta` reset the refusal `stop_details`
- Fixed `grep` path normalization — a hyphenated path segment (`tomte-website\…`) no longer triggers an early split that left backslashes in the Windows result
