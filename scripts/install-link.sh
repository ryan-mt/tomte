#!/usr/bin/env bash
# install-link.sh — Link opencli into $HOME/.local/bin so it auto-syncs when
# you rebuild.
#
# After running this script, every time you edit code and run
# `cargo build --release`, the binary at $HOME/.local/bin/opencli updates
# automatically (because it is a symlink).
#
# Two modes:
#   --release  (default) — symlink to target/release/opencli; you must run
...
if [ "$MODE" = "release" ]; then
  ...
  cat >"$TARGET" <<EOF
#!/usr/bin/env bash
# Auto-rebuild wrapper for opencli. Runs \`cargo run\` on every invocation.
exec cargo run --quiet --manifest-path "$REPO/Cargo.toml" --bin opencli -- "\$@"
EOF
  chmod +x "$TARGET"
  echo "✅ Installed dev wrapper: $TARGET"
  echo "   Every \`opencli\` invocation will rebuild on demand if the source has changed."
fi

echo
echo "Try it now:  opencli --help"
